...
public sealed class EditorRootController : MonoBehaviour
{
    ...
    public void Bind(LevelEditor editor, TopBarView topBar)
    {
    ...
    public void NewLevel(string key)
    {
        ...
        if (!dialogs.ConfirmNew(trimmedKey, state.isDirty))
        {
            Render();
            return;
        }
    ...
    public void LoadLevel(string key)
    {
        ...
        if (!dialogs.ConfirmLoad(trimmedKey, state.isDirty, levelCatalog.GetAssetPath(trimmedKey)))
        {
            Render();
            return;
        }
    ...
    public void SaveCurrentLevel()
    {
        ...
        if (!dialogs.ConfirmSave(trimmedKey, targetExists, levelCatalog.GetAssetPath(trimmedKey)))
        {
            Render();
            return;
        }
