...
public sealed class LevelSessionService
{
    ...
    public bool Save(LevelEditorWorkspace editor, string key, bool targetExists, LevelCatalogService catalog, out string error)
    {
        error = string.Empty;
        if (editor == null)
        {
            error = "LevelEditorWorkspace is missing.";
            return false;
        }
