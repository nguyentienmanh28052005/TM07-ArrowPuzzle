...
public sealed class EditorDialogService
{
    ...
    public int ShowSavePrompt(string levelName)
    {
#if UNITY_EDITOR
        return EditorUtility.DisplayDialogComplex(
            "Save Changes",
            $"Do you want to save changes to the current level '{levelName}' before switching?",
            "Yes (Save)",
            "Cancel",
            "No (Discard)"
        );
#else
        return 2; // Default to Discard / No in non-editor environments
#endif
    }
