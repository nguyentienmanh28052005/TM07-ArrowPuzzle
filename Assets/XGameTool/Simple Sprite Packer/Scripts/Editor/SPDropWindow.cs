...
namespace SimpleSpritePackerEditor
{
    public class SPDropWindow : EditorWindow
    {
        ...
        protected void OnEnable()
        {
            ...
            {
                string instancePath =
                    AssetDatabase.GetAssetPath(EditorPrefs.GetInt(SPTools.Settings_SavedInstanceIDKey, 0));
        ...
        protected void OnGUI()
        {
            ...
            GUI.changed = false;
            this.m_Instance =
                EditorGUILayout.ObjectField("Sprite Packer", this.m_Instance, typeof(SPInstance), false) as SPInstance;
            if (GUI.changed)
            {
                // Save the instance id
                EditorPrefs.SetInt(SPTools.Settings_SavedInstanceIDKey,
                    (this.m_Instance == null) ? 0 : this.m_Instance.GetInstanceID());
            }
