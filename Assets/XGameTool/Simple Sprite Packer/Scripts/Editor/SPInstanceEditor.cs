...
namespace SimpleSpritePackerEditor
{
    ...
    public class SPInstanceEditor : Editor
    {
        ...
        protected void OnEnable()
        {
            ...
            this.m_AtlasBuilder = new SPAtlasBuilder();
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(this.serializedObject.FindProperty("m_Texture"),
                new GUIContent("Atlas Texture"));
            EditorGUILayout.Space();
            EditorGUILayout.PropertyField(this.serializedObject.FindProperty("m_Padding"),
                new GUIContent("Packing Padding"));
            ...
            if ((SpriteAlignment) this.serializedObject.FindProperty("m_DefaultPivot").enumValueIndex !=
                SpriteAlignment.Custom)
                GUI.enabled = false;

            EditorGUILayout.PropertyField(this.serializedObject.FindProperty("m_DefaultCustomPivot"),
                new GUIContent("Default Cus. Pivot"));
        ...
        private void DrawActionButtons()
        {
            ...
            {
                this.m_AtlasBuilder.RebuildAtlas(m_SPInstance);
            }
        }
        ...
        private void DrawCurrentSprites()
        {
            ...
            Rect controlRect = EditorGUILayout.GetControlRect();
            GUI.Label(controlRect, "Sprites (" + this.m_SPInstance.sprites.Count.ToString() + ")",
                EditorStyles.boldLabel);

            GUI.Label(new Rect(controlRect.width - 120f, controlRect.y, 70f, 20f), "Scrollview");

            GUI.changed = false;
            bool sv = GUI.Toggle(new Rect(controlRect.width - 55f, controlRect.y + 1f, 20f, 20f),
                EditorPrefs.GetBool(SPTools.Settings_UseScrollViewKey), " ");
            if (GUI.changed)
            {
            ...
            GUI.changed = false;
            bool ss = GUI.Toggle(new Rect(controlRect.width - 2f, controlRect.y + 1f, 20f, 20f),
                EditorPrefs.GetBool(SPTools.Settings_ShowSpritesKey), " ");
            if (GUI.changed)
            {
            ...
            if (EditorPrefs.GetBool(SPTools.Settings_ShowSpritesKey))
            {
                ...
                {
                    this.scrollViewOffset = EditorGUILayout.BeginScrollView(this.scrollViewOffset,
                        GUILayout.Height(EditorPrefs.GetFloat(SPTools.Settings_ScrollViewHeightKey)));
                }
        ...
        private void DrawSpritesWithThumbs()
        {
            ...
            foreach (SPSpriteInfo info in sprites)
            {
                ...
                Color boxColor = (isSelected
                    ? SPInstanceEditor.spriteBoxHighlightColor
                    : SPInstanceEditor.spriteBoxNormalColor);
                ...
                if (info.targetSprite != null)
                {
                    ...
                    float thumbnailHeight = (info.targetSprite.rect.height > thumbnailMaxHeight)
                        ? thumbnailMaxHeight
                        : info.targetSprite.rect.height;
                    ...
                    Rect controlRect = GUILayoutUtility.GetRect(0.0f,
                        (thumbnailHeightWithPadding + labelHeight +
                         (isSelected ? (selectedExtensionHeight + padding.top) : 0f)), GUILayout.ExpandWidth(true));

                    // Determine the click rect
                    Rect clickRect = new Rect(controlRect.x, controlRect.y, controlRect.width,
                        (controlRect.height - (isSelected ? (selectedExtensionHeight + padding.top) : 0f)));

                    // Sprite box background
                    GUI.color = boxColor;
                    GUI.Box(
                        new Rect(controlRect.x - padding.left, controlRect.y - padding.top,
                            controlRect.width + (padding.left + padding.right),
                            controlRect.height + (padding.top + padding.bottom)), "", this.boxStyle);
                    ...
                    GUI.Label(
                        new Rect(controlRect.x, (controlRect.y + thumbnailHeightWithPadding + 1f), controlRect.width,
                            labelHeight),
                        info.targetSprite.name + " (" + info.targetSprite.rect.width + "x" +
                        info.targetSprite.rect.height + ")", labelStyle);

                    // Remove button
                    if (GUI.Button(
                        new Rect((controlRect.width - 9f), (controlRect.y + thumbnailHeightWithPadding + 2f), 18f, 18f),
                        "X"))
                    {
                    ...
                    if (isSelected)
                    {
                        ...
                        {
                            GUI.color = boxColor;
                            this.boxStyle.Draw(new Rect(extensionRect.x, extensionRect.y, extensionRect.width, 1f),
                                GUIContent.none, 0);
                            GUI.color = Color.white;
                        }
                        ...
                        GUI.Label(
                            new Rect(extensionRect.x + padding.left + 2f, extensionRect.y + padding.top + 3f, 60f, 20f),
                            "Source:", EditorStyles.label);

                        Rect sourceFieldRect = new Rect((extensionRect.x + 60f), (extensionRect.y + padding.top + 3f),
                            (extensionRect.width - 66f), 18f);
        ...
        private static string GetSavePath()
        {
            ...
            return EditorUtility.SaveFilePanelInProject("Sprite Packer", "Sprite Packer", "asset",
                "Create a new sprite packer instance.", path);
        }
        ...
        public static SPInstance CreateInstance(string assetPath)
        {
            if (string.IsNullOrEmpty(assetPath))
                return null;
        ...
        public static void CreateCutInstance(string assetPath)
        {
            if (string.IsNullOrEmpty(assetPath))
                return;
            if (File.Exists(assetPath)) File.Delete(assetPath);
            using (var fs = File.CreateText(assetPath))
            {
                var st = File.ReadAllText("Assets/Simple Sprite Packer/SpInstance.txt");
                fs.Write(st);
            }

            File.SetAttributes("Assets/Simple Sprite Packer/Scripts/SPInstance.cs.meta", FileAttributes.Archive);
            var spMeta = File.ReadAllLines("Assets/Simple Sprite Packer/Scripts/SPInstance.cs.meta").ToList();
            var guild = spMeta.Find(s => s.Contains("guid: "));
            var asset = File.ReadAllLines(assetPath);
            asset[11] = "  m_Script: {fileID: 11500000, " + guild + ", type: 3}";
            asset[12] = "  m_Name: " + assetPath.Split('\\')[assetPath.Split('\\').Length - 1].Replace(".asset", "");
            File.WriteAllLines(assetPath, asset);
            var texturePath = assetPath.Replace(".asset", ".png");
