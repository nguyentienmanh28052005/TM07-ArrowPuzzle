...
//            SPTools.TextureSetReadWriteEnabled(m_Instance.texture, true, false);
        ...
        public void RebuildAtlas(SPInstance m_Instance)
        {
            ...
            if (SPTools.TextureSetReadWriteEnabled(m_Instance.texture, true, false))
            {
                ...
                foreach (SPSpriteInfo si in spriteInfoList)
                {
                    ...
                    else if (si.source is Sprite)
                    {
                        ...
                        if (sourceSprite != null)
                        {
                            ...
                            importData.border = sourceSprite.border;
                            importData.alignment = SpriteAlignment.Custom;
                            importData.pivot = new Vector2(sourceSprite.pivot.x / sourceSprite.rect.width, sourceSprite.pivot.y / sourceSprite.rect.height);
                        }
                    }
                ...
                {
                    uvs = SPUITexturePacker.PackTextures(tempTexture, textures, m_Instance.padding, m_Instance.maxSize);
        ...
        public void ResizeTexture(Texture2D mTexture, bool isAuto, bool isUseUnitySize, Direction direction)
        {
            ...
            if (DrawableTexture.GetImageSize(mTexture, out var width, out var height))
            {
                //if (width != mTexture.width || height != mTexture.height) return;
                if (isUseUnitySize)
                {
                    w = mTexture.width % 4;
                    h = mTexture.height % 4;
            ...
            // if (w != 0 || h != 0)
            {
                string path = AssetDatabase.GetAssetPath(mTexture);
                File.SetAttributes(path + ".meta", FileAttributes.Archive);
                var meta = File.ReadAllText(path + ".meta");
                meta = meta.Replace("isReadable: 0", "isReadable: 1");
                File.WriteAllText(path + ".meta", meta);
                EditorUtility.SetDirty(mTexture);
                AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate);
                try
                {
                    DrawableTexture.PenTexture(mTexture, dir, isUseUnitySize);
                }
                catch (Exception e)
                {
                }
                meta = File.ReadAllText(path + ".meta");
                meta = meta.Replace("isReadable: 1", "isReadable: 0");
                File.SetAttributes(path + ".meta", FileAttributes.Archive);
                File.WriteAllText(path + ".meta", meta);
                EditorUtility.SetDirty(mTexture);
            }
