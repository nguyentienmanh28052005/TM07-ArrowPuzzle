...
//         if (level.turnStateBlocks != null)
//         {
//             for (int i = 0; i < level.turnStateBlocks.Count; i++)
//                 TryOccupy(occupied, level.turnStateBlocks[i].position, "turn-state block " + i, errors);
//         }
...
//     private readonly Dictionary<Vector2Int, bool> _initialTurnStateBlocks = new Dictionary<Vector2Int, bool>();
...
//         if (level.turnStateBlocks != null)
//             foreach (TurnStateBlockSaveData item in level.turnStateBlocks)
//                 _initialTurnStateBlocks[item.position] = item.startsRed;
//     }
...
//         SolveState state = SolveState.Create(_snakes.Count, _keycardColors.Keys, _gateColors.Keys, _buttonColors.Keys, _wallColors.Keys, _initialCountdowns, _initialStopBlocks, _initialTurnStateBlocks);
...
//             if (state.turnStateBlocks.TryGetValue(next, out bool isRedTurnState) && isRedTurnState) return false;
...
//         ToggleTurnStateBlocks(nextState);
//         return nextState;
//     }
...
//     private void ToggleTurnStateBlocks(SolveState state)
//     {
