...
public class UIAtlasMaker : EditorWindow
{
	...
	static public SpriteEntry DuplicateSprite (INGUIAtlas atlas, string spriteName)
	{
		...
		else NGUIEditorTools.ImportTexture(atlas.texture, false, false, true);
		return se;
	}
	...
	static public SpriteEntry ExtractSprite (INGUIAtlas atlas, string spriteName)
	{
		...
		NGUIEditorTools.ImportTexture(atlas.texture, false, false, true);
		return se;
	}
	...
	static public void ExtractSprites (INGUIAtlas atlas, List<SpriteEntry> finalSprites)
	{
		...
		NGUIEditorTools.ImportTexture(atlasTexture, false, false, true);
		ShowProgress(1f);
	}
	...
	static public bool UpdateTexture (INGUIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (PackTextures(tex, sprites))
		{
			...
			tex = NGUIEditorTools.ImportTexture(newPath, false, true, true);
		...
		{
			if (!newTexture) NGUIEditorTools.ImportTexture(oldPath, false, true, false);
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				foreach (KeyValuePair<string, int> iter in spriteList)
				{
					...
					{
						selection = iter.Key;
						if (NGUISettings.selectedSprite != selection)
						{
							NGUISettings.selectedSprite = selection;
							NGUISettings.selectedSpriteChange?.Invoke();
						}
					}
