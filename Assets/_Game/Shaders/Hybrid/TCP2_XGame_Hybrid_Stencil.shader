// Custom Hybrid Shader - Based on Toony Colors Pro 2
// Standalone replacement with identical properties for drop-in material compatibility

Shader "XGame/TCP2 Hybrid Stencil"
{
	Properties
	{
	...
	//# Stencil
		...
		[Enum(UnityEngine.Rendering.CompareFunction)] _StencilComp ("Compare", Float) = 0
		[Enum(UnityEngine.Rendering.StencilOp)] _StencilPass ("Pass Op", Float) = 0
		[Enum(UnityEngine.Rendering.StencilOp)] _StencilFail ("Fail Op", Float) = 0
		[Enum(UnityEngine.Rendering.StencilOp)] _StencilZFail ("ZFail Op", Float) = 0
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			#include "TCP2_XGame_Hybrid_Include.cginc"
	...
	CustomEditor "CustomTCP2HybridInspector"
}
