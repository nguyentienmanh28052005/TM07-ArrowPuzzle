Shader "XGame/Surface-Pinky-Outline"
{
    Properties
    {
        ...
        _LightIntensity("Light Intensity (Smoothness)", Range(0,1)) = 0.90
        _Shininess("Shininess (Spec Power)", Range(1, 100)) = 80
        _SpecularColor("Specular Color", Color) = (0.98, 1.0, 1.0, 1)

        _WrapBase("Wrap Base", Range(0,1)) = 0.45
        _WrapScale("Wrap Scale (by _LightIntensity)", Range(0,1)) = 0.35
        ...
        _RimLit("Rim By _LightIntensity", Range(0,2)) = 0.45
        ...
        _ZSmooth("Outline Z Smoothing", Range(0,1)) = 0.0
        [Toggle] _OutlineConstSize("Outline Constant Pixel Size", Float) = 0
    }
    ...
    SubShader
    {
        ...
        #pragma target 3.0
        #pragma multi_compile_instancing   // chỉ 1 lần
        #pragma instancing_options assumeuniformscaling

        sampler2D _MainTex;
        sampler2D _BumpMap;

        // không khai báo uniform _Color thường, dùng instancing
        half _BumpScale;
        ...
        // toon
        half _ToonSteps, _SpecSteps, _RampSmooth;
        half _RampFeather;
        half _ToonBlend;

        // surface lighting controls
        float4 _FaceLightDir;
        half _FaceLightWeight;
        half _ToonBias;
        fixed4 _AmbientColor;
        half _OverallBrightness;

        // instancing buffer cho _Color (giữ tên)
        ...
        // Safe normalize to avoid division-by-zero in Meta/other passes
        ...
        // Smooth toon quantization using screen-space derivatives
        inline float SmoothToon(float v, float steps, float smooth, float feather)
        {
            float x = saturate(v) * steps;
            float q0 = floor(x);
            float center = q0 + 0.5;
            // derivative-based width to keep edges stable across resolutions
            float fw = fwidth(x);
        ...
        inline half4 LightingCustomHalfLambert(SurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
        {
            ...
            // Wrap Half-Lambert (an toàn chia cho 0)
            float wrap = _WrapBase + lit * _WrapScale; // có thể âm nếu bạn kéo quá tay
            float numer = dot(N, L) + wrap;
            float denom = max(0.001, 1.0 + wrap); // <-- FIX: tránh 1+wrap = 0
            float NdotLw = saturate(numer / denom);
            float hLambert = NdotLw * (_DiffBase + lit * _DiffLit) + _DiffFloor + (float)_ToonBias;
            // Smooth toon quantization for diffuse
            ...
            float diffuseFinal = lerp(diffuseRaw, qd, (float)_ToonBlend);
            ...
            // Build both smooth and toon diffuse colors
            ...
            // Final blend between smooth and toon
            float3 col = lerp(colSmooth + specSmooth * atten, colToon + specToon * atten, (float)_ToonBlend);
            col += rim + glow;

            // Pastel grading (an toàn chia cho 0)
            float luma = dot(col, float3(0.299, 0.587, 0.114));
            col = lerp(luma.xxx, col, _SatGain);
            float invGamma = 1.0 / max(0.001, (float)_GammaLift); // <-- FIX
            col = pow(saturate(col), invGamma);

            // Áp dụng điều chỉnh ánh sáng tổng thể
            col *= (float)_OverallBrightness;

            return half4(saturate(col), s.Alpha);
        }
        ...
        // Optional Outline Pass (front-face cull + vertex extrusion). Bật bằng _UseOutline > 0
        Pass
        {
            Name "Outline"
            Tags
            ...
            Cull Front
            Offset 2, 2 // ✅ push outline out of z-buffer (fix z-fighting on mobile)
            ...
            UNITY_INSTANCING_BUFFER_START(Props)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float, _ZSmooth)
                UNITY_DEFINE_INSTANCED_PROP(float4, _OutlineColor)
                UNITY_DEFINE_INSTANCED_PROP(float, _OutlineConstSize)
            UNITY_INSTANCING_BUFFER_END(Props)
            ...
            // ✅ Safe normalize for mobile precision
            inline float3 SafeNormalize(float3 v)
            {
            ...
            v2f vert(appdata v)
            {
                ...
                // Safe normal → view-space
                float3 nVS = mul((float3x3)UNITY_MATRIX_IT_MV, v.normal);
                nVS = SafeNormalize(nVS);

                float zSmooth = UNITY_ACCESS_INSTANCED_PROP(Props, _ZSmooth);
                nVS.z = lerp(nVS.z, -abs(nVS.z), zSmooth); // ✅ prevent inverted z

                float4 posVS = mul(UNITY_MATRIX_MV, v.vertex);
                float outline = UNITY_ACCESS_INSTANCED_PROP(Props, _Outline);
                float constSize = UNITY_ACCESS_INSTANCED_PROP(Props, _OutlineConstSize);

                if (constSize > 0.5)
                {
                    // ✅ More stable constant-size outline
                    float dist = max(0.001, -posVS.z);
                    posVS.xyz += nVS * outline * (0.0015 * dist);
                }
                else
                {
                    // ✅ Regular screen-proportional outline
                    posVS.xyz += nVS * outline * 0.01;
                }
            ...
            {
                fixed4 col = UNITY_ACCESS_INSTANCED_PROP(Props, _OutlineColor);
                return col;
            }
            ENDCG
        }
