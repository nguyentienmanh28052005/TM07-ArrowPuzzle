...
namespace UnityEngine.UI
{
    ...
    /// </remarks>
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler,
        IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        /// </summary>
        protected LoopScrollPrefabSource prefabSource;
        ...
        /// </summary>
        public int itemTypeStart { get; protected set; }

        /// <summary>
        /// The last item id in LoopScroll.
        /// </summary>
        public int itemTypeEnd { get; protected set; }
        ...
        protected int contentConstraintCount
        {
            get
            {
                ...
                if (ContentPool != null)
                {
                    GridLayoutGroup layout2 = ContentPool.GetComponent<GridLayoutGroup>();
                    if (layout2 != null)
                    {
        ...
        [SerializeField] protected RectTransform m_Content; //==========LoopScrollRect==========
        ...
        private Action<Transform, int> m_OnProvideData;

        public Action<Transform, int> onProvideData
        {
            get => m_OnProvideData;
            set => m_OnProvideData = value;
        }
        ...
        public void ClearCells()
        {
            if (Application.isPlaying && prefabSource != null)
            {
                ...
                itemTypeEnd = 0;
                totalCount = 0;
                for (int i = ContentPool.childCount - 1; i >= 0; i--)
                {
                    prefabSource.ReturnObject(ContentPool.GetChild(i));
                }
            }
        }
        ...
        public void SetCanProvideData(bool canProvideData)
        {
            this.canProvideData = canProvideData;
        }

        private Coroutine Coroutine;

        public void ScrollToCell(int index, float speed, Action onDone, int spaceLast)
        {
            ...
            Coroutine = StartCoroutine(ScrollToCellCoroutine(index, speed, onDone, spaceLast));
        }

        public void ScrollToCellWithinTime(int index, float time, Action onDone)
        {
            if (totalCount >= 0 && (index < 0 || index >= totalCount))
            {
                Debug.LogErrorFormat("invalid index {0}", index);
            ...
            {
                float elementSize = (GetAbsDimension(m_ContentBounds.size) - contentSpacing * (CurrentLines - 1)) /
                                    CurrentLines;
            ...
            StartCoroutine(ScrollToCellCoroutine(index, dist / time, onDone));
        }

        IEnumerator ScrollToCellCoroutine(int index, float speed, Action onDone, int spaceLast = 0)
        {
                ...
                if (!m_Dragging)
                {
                    ...
                    else
                    {
                        ...
                        if (totalCount >= 0)
                        {
                            ...
                            {
                                m_ItemBounds = GetBoundsLastItem() /* : GetBounds4Item(totalCount - 1)*/;
                                if (spaceLast > 0)
                                {
                                    if (direction == LoopScrollRectDirection.Vertical)
                                    {
                                        m_ItemBounds.size = m_ItemBounds.size + new Vector3(0,
                                            GetSpaceSize(prefabSource.item) * spaceLast, 0);
                                    }
                                    else
                                    {
                                        m_ItemBounds.size = m_ItemBounds.size +
                                                            new Vector3(GetSpaceSize(prefabSource.item) * spaceLast, 0,
                                                                0);
                                    }
                                }
        ...
        public void ScrollObj(RectTransform objMove, int startIndex, int endIndex, float speedRate,
            AnimationCurve speedCurve, Action<int> onDone)
        {
            StopAllCoroutines();
            StartCoroutine(scrollObj(objMove, startIndex, endIndex, speedRate, speedCurve, onDone));
        }

        IEnumerator scrollObj(RectTransform objMove, int startIndex, int endIndex, float speedRate,
            AnimationCurve speedCurve, Action<int> onDone)
        {
            bool needMoving = true;
            var del = endIndex - startIndex;
            var speed = 0f;
            var totalMove = Mathf.Abs(del);
            var directionMove = Mathf.Sign(del);
            var s = totalMove * GetSize(prefabSource.item.GetComponent<RectTransform>(), true);
            var sMove = 0f;
            while (needMoving)
            {
                yield return null;
                speed = speedCurve.Evaluate(sMove / s) * speedRate;
                float move = 0;
                if (directionMove < 0)
                {
                    move = -Time.deltaTime * speed;
                ...
                if (Mathf.Abs(sMove + move) >= s)
                {
                    move = directionMove * (s - sMove);
                }
            ...
            StopMovement();
            UpdatePrevData();
            // move obj to targetItem
            yield break;
            var canvas = UIManager.Instance.canvas;
            int childIndex = Mathf.Clamp(endIndex - itemTypeStart, 0, m_Content.childCount - 1);
            var targetItem = m_Content.GetChild(childIndex) as RectTransform;
            var screenPoint =
                RectTransformUtility.WorldToScreenPoint(canvas.worldCamera, targetItem.transform.position);
            RectTransformUtility.ScreenPointToLocalPointInRectangle(viewport, screenPoint, canvas.worldCamera,
                out var targetPosition);
            objMove.anchoredPosition = targetPosition;

            /*var dMove = (targetPosition - objMove.anchoredPosition);
              needMoving = true;
            var directionAxis = direction == LoopScrollRectDirection.Vertical ? dMove.y : dMove.x;
            directionAxis = reverseDirection ? directionAxis * -1 : directionAxis;
            var sObjMove = 0f;
            var length = Mathf.Abs(directionAxis);
            while (needMoving)
            {
                yield return null;
                speed = Mathf.Lerp(speed, speed * 0.8f, 0.5f);
                float move = 0;
                if (directionAxis < 0)
                {
                    move = -Time.deltaTime * speed;
                ...
                if (Mathf.Abs(sObjMove + move) >= length)
                {
                    move = directionAxis * (length - sObjMove);
                }
                sObjMove += move;
                Vector2 offset = GetVector(move);
                objMove.anchoredPosition += offset;
                if (Mathf.Abs(sObjMove) >= length)
                {
                    needMoving = false;
                    objMove.anchoredPosition = targetPosition;
                    break;
            ...
            // move down all item below targetItem
            var sq = DOTween.Sequence();
            sq.SetId(this);
            var spa = GetSpaceSize(prefabSource.item);
            for (int i = childIndex; i < m_Content.childCount; i++)
            {
                var child = m_Content.GetChild(i).GetComponent<RectTransform>();
                var endPos = new Vector2(child.anchoredPosition.x, child.anchoredPosition.y - spa);
                sq.Insert(0.2f, child.DOAnchorPos(endPos, 0.5f));
            }

            sq.OnComplete(() => { onDone?.Invoke(childIndex); });
        }

        private bool canProvideData;

        protected virtual void ProvideData(Transform transform, int index)
        {
            if (!canProvideData) return;
            m_OnProvideData?.Invoke(transform, index);
        }

        public virtual void Initialized(int count, GameObject prefabItem, Action<Transform, int> provideData = null,
            int startItem = 0, bool fillViewRect = false, float contentOffset = 0, bool isUpdate = true)
        {
            IsUpdate = isUpdate;
            canProvideData = true;
            m_OnProvideData = provideData;
            this.totalCount = count;
            prefabSource ??= new LoopScrollPrefabSource(prefabItem, this);
            RefillCells(startItem, fillViewRect, contentOffset);
        }
        ...
        {
            return GetAbsDimension(viewRect.rect.size);
        }

        public float GetSpaceSize(GameObject prefabItem)
        {
            return GetSize(prefabItem.GetComponent<RectTransform>());
        }

        public Vector2 GetSpaceVector(float value)
        {
        ...
        {
            return m_Content.GetComponent<GridLayoutGroup>().padding;
        }

        public float GetContentSize()
        {
            return GetAbsDimension(m_ContentBounds.size);
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                itemTypeEnd = itemTypeStart;
                // recycle items if we can
                for (int i = 0; i < ContentPool.childCount; i++)
                {
                    if (itemTypeEnd < totalCount)
                    {
                        ProvideData(m_Content.GetChild(i), itemTypeEnd);
        ...
        public void RefillCellsFromEnd(int endItem = 0, bool alignStart = false)
        {
            ...
            itemTypeEnd = reverseDirection ? totalCount - endItem : endItem;
        ...
        public void RefillCells(int startItem = 0, bool fillViewRect = false, float contentOffset = 0)
        {
            ...
            ReturnToTempPool(reverseDirection, ContentPool.childCount);
            ...
            if (sizeToFill > sizeFilled && sizeFilled > GetAbsDimension(viewRect.rect.size))
            {
                contentOffset += sizeToFill - sizeFilled;
                sizeToFill = sizeFilled;
            }
            // refill from start in case not full yet
            // while (sizeToFill > sizeFilled)
            // {
        ...
        protected float NewItemAtEnd(bool includeSpacing = true)
        {
            ...
            int availableChilds = ContentPool.childCount - deletedItemTypeStart - deletedItemTypeEnd;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                newItem.SetSiblingIndex(ContentPool.childCount - deletedItemTypeEnd - 1);
                size = Mathf.Max(GetSize(newItem, includeSpacing), size);
                itemTypeEnd++;
                if (totalCount >= 0 && itemTypeEnd >= totalCount)
                {
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                RectTransform oldItem =
                    ContentPool.GetChild(ContentPool.childCount - deletedItemTypeEnd - 1) as RectTransform;
                size = Mathf.Max(GetSize(oldItem), size);
        ...
        void UpdateScrollbarLayout()
        {
            if (m_VSliderExpand && m_HorizontalScrollbar)
            {
                m_Tracker.Add(this, m_HorizontalScrollbarRect,
                    DrivenTransformProperties.AnchorMinX | DrivenTransformProperties.AnchorMaxX |
        ...
        {
            int offset = Mathf.Clamp(index - itemTypeStart, 0, m_Content.childCount - 1);
        ...
        private Bounds GetBoundsLastItem()
        {
            if (m_Content == null)
                return new Bounds();
            var rt = m_Content.GetChild(m_Content.childCount - 1) as RectTransform;
            if (rt == null)
                return new Bounds();
            rt.GetWorldCorners(m_Corners);
        ...
        internal static Vector2 InternalCalculateOffset(ref Bounds viewBounds, ref Bounds contentBounds,
            bool horizontal, bool vertical, MovementType movementType, ref Vector2 delta, float deltaDimension,
            int itemTypeStart, int itemTypeEnd, int totalCount) //==========LoopScrollRect==========
        {
        ...
        public RectTransform ContentPool
        {
            get
            {
                if (contentPool == null)
        ...
        [SerializeField] private RectTransform contentPool;
    }
}