    ...
    public abstract class LoopScrollRect : LoopScrollRectBase
    {
        protected override RectTransform GetFromTempPool(int itemIdx)
        {
            ...
            if (deletedItemTypeStart > 0)
            {
                ...
                nextItem = ContentPool.GetChild(0) as RectTransform;
                nextItem.SetSiblingIndex(itemIdx - itemTypeStart + deletedItemTypeStart);
            }
            else if (deletedItemTypeEnd > 0)
            {
                deletedItemTypeEnd--;
                nextItem = ContentPool.GetChild(ContentPool.childCount - 1) as RectTransform;
                nextItem.SetSiblingIndex(itemIdx - itemTypeStart + deletedItemTypeStart);
            }
            else
            {
                nextItem = prefabSource.GetObject().transform as RectTransform;
                nextItem.transform.SetParent(ContentPool, false);
        ...
        {
            Debug.Assert(ContentPool.childCount >= deletedItemTypeStart + deletedItemTypeEnd);
            if (deletedItemTypeStart > 0)
            {
            ...
            {
                int t = ContentPool.childCount - deletedItemTypeEnd;
                for (int i = ContentPool.childCount - 1; i >= t; i--)
                {
                    prefabSource.ReturnObject(ContentPool.GetChild(i));
                }
