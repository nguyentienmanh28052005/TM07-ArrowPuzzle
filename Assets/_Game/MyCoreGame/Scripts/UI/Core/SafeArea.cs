...
namespace Crystal
{
    ...
    public class SafeArea : master.Singleton<SafeArea>
    {
        ...
        private void RegisterListener()
        {
            ...
            var ob_shopPack = Observer.GetObservable(ObserverName.purchase_success, 0);
            subShopPack = ob_shopPack.Subscribe(x => { Refresh(); });
            var ob1 = Observer.GetObservable(ObserverName.screen_resize, 0);
            sub = ob1.Subscribe(x =>
            {
                if (SafeAreaFolding.IsFoldingScreen())
                {
                    ConformY = false;
                }
                else
        ...
        void Refresh()
        {
            isRemoveAds = true;//AdsHelper.isRemoveAds(0);
        ...
        void ApplySafeArea(Rect r)
        {
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                var heightArea = h + (1 - h) / 3f;
                
                // Convert safe area rectangle from absolute pixels to normalised anchor coordinates
                Vector2 anchorMin = r.position;
                Vector2 anchorMax = r.position + r.size;
                anchorMin.x /= Screen.width;
                anchorMin.y = 0;//= Screen.height;
                anchorMax.x /= Screen.width;
                anchorMax.y = heightArea;
                ...
                if (anchorMin.x >= 0 && anchorMin.y >= 0 && anchorMax.x >= 0 && anchorMax.y >= 0)
                {
                    ...
                    {
                        SetupPanel(Panel[i], anchorMin, anchorMax, height);
                    }
                }
            }
        ...
        public void SetupPanel(SafeAreaPanel panel, Vector2 anchorMin, Vector2 anchorMax, float heightBanner)
        {
            panel.rectTransform.anchorMin = anchorMin;
            panel.rectTransform.anchorMax = anchorMax;
            if (panel.isSetBanner && !isRemoveAds)
            {
                panel.rectTransform.anchorMin = new Vector2(0, heightBanner / Screen.height);
            }
            bool isSafeTop = false;
            if (panel.IsSafeTop == SafeAreaPanel.Platform.All)
            {
                isSafeTop = true;
            }
            else if (panel.IsSafeTop == SafeAreaPanel.Platform.None)
            {
                isSafeTop = false;
            }
            else
            {
#if UNITY_ANDROID
                if (panel.IsSafeTop == SafeAreaPanel.Platform.Android)
                {
                    isSafeTop = true;
                }
#else
                if (panel.IsSafeTop == SafeAreaPanel.Platform.iOS)
                {
                    isSafeTop = true;
                }
#endif
            ...
            if (!isSafeTop)
            {
                panel.rectTransform.anchorMax = new Vector2(anchorMax.x, 1);
            }
        }
