...
{
    static public string Serialize<T>(IList<T> array)
    {
        var str = "";
        Type type = typeof(T);
        ...
        {
            for (int field = 0; field < fieldinfo.Length; field++)
            {
                str += GetStringValue(array[i], fieldinfo[field]);
                if (field != fieldinfo.Length - 1)
                {
                    str += ",";
                }
            }
    ...
    static string GetStringValue(object v, FieldInfo fieldinfo)
    {
        var str = "";
        if (fieldinfo.FieldType.IsArray)
        {
            System.Collections.IList array = (System.Collections.IList)fieldinfo.GetValue(v);
            if (array != null)
            {
                for (int i = 0; i < array.Count; i++)
                {
