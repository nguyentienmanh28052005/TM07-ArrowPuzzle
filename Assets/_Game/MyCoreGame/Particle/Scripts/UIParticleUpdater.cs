...
namespace Coffee.UIExtensions
{
    internal static class UIParticleUpdater
    {
        ...
        private static void Refresh()
        {
            ...
            // updateRender(s_ActiveParticles);
        ...
        private static void updateRender(List<UIParticle> s_ActiveParticles)
        {
            for (var i = 0; i < s_ActiveParticles.Count; i++)
            {
                var uip = s_ActiveParticles[i];
                if (!uip || !uip.canvas) continue;
                uip.UpdateTransformScale();
                if (uip.isPrimary && !s_UpdatedGroupIds.Contains(uip.groupId))
                {
                    s_UpdatedGroupIds.Add(uip.groupId);
                    uip.UpdateRenderers();
                }
                else if (!uip.useMeshSharing)
                {
                    uip.UpdateRenderers();
                }
                else if (!s_UpdatedGroupIds.Contains(uip.groupId))
